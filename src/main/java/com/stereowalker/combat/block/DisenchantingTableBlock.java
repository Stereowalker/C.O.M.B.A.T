...
import com.stereowalker.combat.inventory.container.DisenchantmentContainer;
...
public class DisenchantingTableBlock extends ContainerBlock {
	protected static final VoxelShape SHAPE = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 12.0D, 16.0D);

	protected DisenchantingTableBlock(Block.Properties builder) {
		super(builder);
	}
	...
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return new DisenchantingTableTileEntity();
	}
	...
	public INamedContainerProvider getContainer(BlockState state, World worldIn, BlockPos pos) {
		...
		if (tileentity instanceof DisenchantingTableTileEntity) {
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		if (stack.hasDisplayName()) {
			TileEntity tileentity = worldIn.getTileEntity(pos);
			if (tileentity instanceof DisenchantingTableTileEntity) {
				((DisenchantingTableTileEntity)tileentity).setCustomName(stack.getDisplayName());
			}
		}
