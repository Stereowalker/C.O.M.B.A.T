...
public class PyraniteFireBlock extends AbstractFireBlock {
   ...
   public PyraniteFireBlock(AbstractBlock.Properties builder) {
      super(builder, 1.0F);
      ...
      this.stateToShapeMap = ImmutableMap.copyOf(this.stateContainer.getValidStates().stream().filter((state) -> {
       ...
       }).collect(Collectors.toMap(Function.identity(), PyraniteFireBlock::getShapeForState)));
    }
   ...
   public static void init() {
      ...
      fireblock.setFireInfo(CBlocks.MEZEPINE, 60, 100);
      fireblock.setFireInfo(CBlocks.PYRANITE_ORE, 60, 100);
