...
public class ChakramItem extends TieredItem {
	private final float attackDamage;
	/** Modifiers applied when the item is in the mainhand of a user. */
	private final Multimap<Attribute, AttributeModifier> attributeModifiers;

	public ChakramItem(IItemTier tier, int attackDamageIn, float attackSpeedIn, Item.Properties builderIn) {
		super(tier, builderIn);
		this.attackDamage = (float)(attackDamageIn*1.1F) + tier.getAttackDamage();
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (i >= 10 - (CEnchantmentHelper.getShortThrowModifier(stack) * 3)) {
				if (!worldIn.isRemote) {
					...
					ChakramEntity entityChakram = new ChakramEntity(worldIn, entityplayer, stack);
					entityChakram.setDirectionAndMovement(entityplayer, entityplayer.rotationPitch, entityplayer.rotationYaw, 0.0F, 2.5F + (float)0.5F, 1.0F);
					if (entityplayer.abilities.isCreativeMode) {
						entityChakram.pickupStatus = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
					}

					worldIn.addEntity(entityChakram);
					if (!entityplayer.abilities.isCreativeMode) {
