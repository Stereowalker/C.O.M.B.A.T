...
public interface IDyeableWeaponItem {
   ...
   default int getColor(ItemStack stack) {
      CompoundNBT compoundnbt = stack.getChildTag("display");
      return compoundnbt != null && compoundnbt.contains("color", 99) ? compoundnbt.getInt("color") : defaultColor(stack);
   }
   ...
   abstract int defaultColor(ItemStack stack);
   abstract boolean usesDyeingRecipe();
   ...
   static ItemStack dyeItem(ItemStack stack, List<DyeItem> dyes) {
      ...
      int i = 0;
      int j = 0;
      IDyeableWeaponItem idyeablearmoritem = null;
      Item item = stack.getItem();
      if (item instanceof IDyeableWeaponItem) {
         idyeablearmoritem = (IDyeableWeaponItem)item;
