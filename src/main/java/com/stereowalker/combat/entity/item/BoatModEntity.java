...
public class BoatModEntity extends Entity {
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Boolean> LEFT_PADDLE = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> RIGHT_PADDLE = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(BoatModEntity.class, DataSerializers.VARINT);
	...
	private BoatModEntity.Status status;
	private BoatModEntity.Status previousStatus;
	...
	public BoatModEntity(EntityType<? extends BoatModEntity> p_i50129_1_, World world) {
		super(p_i50129_1_, world);
		this.preventEntitySpawning = true;
	}

	public BoatModEntity(World worldIn, double x, double y, double z) {
		this(CEntityType.BOAT, worldIn);
	...
	protected void registerData() {
		...
		this.dataManager.register(BOAT_TYPE, BoatModEntity.Type.AUSLDINE.ordinal());
	...
	public void applyEntityCollision(Entity entityIn) {
		if (entityIn instanceof BoatModEntity) {
	...
	public void tick() {
		...
		if (this.status != BoatModEntity.Status.UNDER_WATER && this.status != BoatModEntity.Status.UNDER_FLOWING_WATER) {
	 ...
	 */
	private BoatModEntity.Status getBoatStatus() {
		BoatModEntity.Status boatentity$status = this.getUnderwaterStatus();
		if (boatentity$status != null) {
			this.waterLevel = this.getBoundingBox().maxY;
			return boatentity$status;
		} else if (this.checkInWater()) {
			return BoatModEntity.Status.IN_WATER;
		} else {
			float f = this.getBoatGlide();
			if (f > 0.0F) {
				this.boatGlide = f;
				return BoatModEntity.Status.ON_LAND;
			} else {
				return BoatModEntity.Status.IN_AIR;
			}
		}
	}
	 ...
	 */
	@Nullable
	private BoatModEntity.Status getUnderwaterStatus() {
		AxisAlignedBB axisalignedbb = this.getBoundingBox();
				...
				for(int i2 = i1; i2 < j1; ++i2) {
					...
					if (fluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$mutable.getY() + fluidstate.getActualHeight(this.world, blockpos$mutable))) {
						if (!fluidstate.isSource()) {
							return BoatModEntity.Status.UNDER_FLOWING_WATER;
						}
		...
		return flag ? BoatModEntity.Status.UNDER_WATER : null;
	}
	...
	private void updateMotion() {
		...
		double d2 = 0.0D;
		this.momentum = 0.05F;
		if (this.previousStatus == BoatModEntity.Status.IN_AIR && this.status != BoatModEntity.Status.IN_AIR && this.status != BoatModEntity.Status.ON_LAND) {
			...
			this.lastYd = 0.0D;
			this.status = BoatModEntity.Status.IN_WATER;
		} else {
			if (this.status == BoatModEntity.Status.IN_WATER) {
				d2 = (this.waterLevel - this.getPosY()) / (double)this.getHeight();
				this.momentum = 0.9F;
			} else if (this.status == BoatModEntity.Status.UNDER_FLOWING_WATER) {
				d1 = -7.0E-4D;
				this.momentum = 0.9F;
			} else if (this.status == BoatModEntity.Status.UNDER_WATER) {
				d2 = (double)0.01F;
				this.momentum = 0.45F;
			} else if (this.status == BoatModEntity.Status.IN_AIR) {
				this.momentum = 0.9F;
			} else if (this.status == BoatModEntity.Status.ON_LAND) {
	...
	protected void readAdditional(CompoundNBT compound) {
		if (compound.contains("Type", 8)) {
			this.setBoatType(BoatModEntity.Type.getTypeFromString(compound.getString("Type")));
		}
		...
		if (!this.isPassenger()) {
			if (onGroundIn) {
				if (this.fallDistance > 3.0F) {
					if (this.status != BoatModEntity.Status.ON_LAND) {
	...
	public void setBoatType(BoatModEntity.Type boatType) {
		this.dataManager.set(BOAT_TYPE, boatType.ordinal());
	}

	public BoatModEntity.Type getBoatType() {
		return BoatModEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
	}
	...
	public boolean canSwim() {
		return this.status == BoatModEntity.Status.UNDER_WATER || this.status == BoatModEntity.Status.UNDER_FLOWING_WATER;
	}
	...
	public static enum Type {
		 ...
		 */
		public static BoatModEntity.Type byId(int id) {
			BoatModEntity.Type[] aentityboat$type = values();
		...
		public static BoatModEntity.Type getTypeFromString(String nameIn) {
			BoatModEntity.Type[] aentityboat$type = values();
