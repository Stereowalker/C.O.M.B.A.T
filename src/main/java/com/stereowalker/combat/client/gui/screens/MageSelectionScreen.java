...
public class MageSelectionScreen extends Screen {
	...
	public MageSelectionScreen(Minecraft mc, Screen previousScreen) {
		super(Component.translatable(Combat.getInstance().getModid()+".mage_creation.title"));
		this.previousScreen = previousScreen;
		this.minecraft = mc;
	}

	public Button create(int x, int y, SpellCategory cat) {
		return this.addRenderableWidget(ScreenHelper.buttonBuilder(cat.getColoredDisplayName(), (onPress) -> {
		...
		}).bounds(this.width / 2 + x, this.height / 6 + y, 80, 20).build());
	}
	...
	public void tick() {
		...
		for (Slider slider : sliders) {
			...
			MutableComponent component = Component.translatable("gui."+(i==0?"red":i==1?"green":"blue")+"_slider").append(": " + s);
	...
	public void render(PoseStack matrixStack, int p_render_1_, int p_render_2_, float p_render_3_) {
		...
		GuiComponent.drawCenteredString(matrixStack, this.font, Component.translatable("gui.elemental_affinity"), this.width / 2, this.height / 6, 16777215);
		GuiComponent.drawCenteredString(matrixStack, this.font, Component.translatable("gui.primeval_affinity"), this.width / 2, this.height / 6 + 43, 16777215);
		GuiComponent.drawCenteredString(matrixStack, this.font, Component.translatable("gui.mana_color"), this.width / 2, this.height / 6 + 82, 16777215);
