...
public class AcrotlestStraightTrunkPlacer extends AbstractTrunkPlacer {
   public static final Codec<AcrotlestStraightTrunkPlacer> CODEC = RecordCodecBuilder.create((builderInstance) -> {
      return getAbstractTrunkCodec(builderInstance).apply(builderInstance, AcrotlestStraightTrunkPlacer::new);
   });

   public AcrotlestStraightTrunkPlacer(int baseHeight, int heightRandA, int heightRandB) {
   ...
   protected static void func_236909_a_(IWorldGenerationReader reader, BlockPos pos) {
      if (!func_236912_a_(reader, pos)) {
         TreeFeature.setBlockStateWithoutUpdate(reader, pos, CBlocks.PURIFIED_DIRT.getDefaultState());
      }
   }
}