...
public class AcrotlestStraightTrunkPlacer extends TrunkPlacer {
	public static final Codec<AcrotlestStraightTrunkPlacer> CODEC = RecordCodecBuilder.create((builderInstance) -> {
		return trunkPlacerParts(builderInstance).apply(builderInstance, AcrotlestStraightTrunkPlacer::new);
	});
	...
	private static boolean isDirt(LevelSimulatedReader pLevel, BlockPos pPos) {
		return pLevel.isStateAtPosition(pPos, (p_70304_) -> {
			return p_70304_.is(CBlocks.PURIFIED_DIRT) && !p_70304_.is(CBlocks.PURIFIED_GRASS_BLOCK);
		});
	}
